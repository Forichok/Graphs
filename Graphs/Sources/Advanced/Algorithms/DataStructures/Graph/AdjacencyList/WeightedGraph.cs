...
namespace Graphs.Sources.Advanced.Algorithms.DataStructures.Graph.AdjacencyList
{
    ...
    public class WeightedGraph<T, TW> : IEnumerable<T> where TW : IComparable
    {
        ...
        public static WeightedGraph<string, int> GetGraph(IEnumerable<LinkModel> links, IEnumerable<NodeModel> nodes)
        {
            var newGraph = new WeightedGraph<string, int>();

            foreach (var node in nodes)
            {
                newGraph.AddVertex(node.Key);
            }
